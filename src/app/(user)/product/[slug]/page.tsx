...
const SingleProductPage = async ({params} : Props) => {
  ...
  return (
    <Container className='my-10 bg-beLight'>
        ...
        <div className='w-full md:col-span-2 xl:col-span-3 xl:p-14 flex flex-col gap-6 justify-center'>
            ...
            <div className='flex items-center gap-4'>
                    ...
                    <p className='line-through text-lg font-normal text-gray-500'>
                        ...
                        <FormatedPrice amount={product.rowprice} className='text-lg font-bold' /></p>
                        <FormatedPrice amount={product.price} className='text-lg font-bold' />
                    <p className='text-sm'>You Saved {" "}
                        <FormatedPrice amount={product?.rowprice - product.price} 
                        className='text-white px-2 rounded-md text-xs
                         font-bold bg-lightGreen py-1' /> {" "} From This Item
                    </p>
          ...
          {Array.from({ length: 5 }).map((_, index) => {
            ...
            const halfFilled =
              index + 1 > Math.ceil(product.rating) && index < 
